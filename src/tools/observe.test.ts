...
describe("Observe Tool End-to-End", () => {
  let stagehandInstance: Awaited<ReturnType<typeof getStagehand>> | null = null;
  let observeHandler: any;

  beforeAll(async () => {
    stagehandInstance = await getStagehand();

    // Ensure we are on a page where we can observe elements
    ...
    registerObserveTool(mockServer);
  }, 60000);
  ...
  it("should successfully observe elements on the page and create a recording", async () => {
    expect(observeHandler).toBeDefined();

    const timestampStart = Date.now();
    const result = await observeHandler({});
